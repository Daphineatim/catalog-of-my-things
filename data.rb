...
def save_music_album(genre, author, source, label, publish_date, on_spotify)
  obj = {
    genre: genre,
    author: author,
    source: source,
...
def load_music_album(file="./json/music_album.json")
  if File.exist?('./json/music_album.json')
    open_album=File.open('./json/music_album.json')
    if open_album.size.zero?
      puts "no music albums"
    else
      read_album=JSON.parse(File.read('./json/music_album.json'))
      read_album.each do |music_albums|
        music_albums=MusicAlbum.new(music_albums['genre'], music_albums['author'], music_albums['source'],music_albums['label'], music_albums['publish_date'], music_albums['on_spotify'])
